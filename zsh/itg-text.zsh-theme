...
function precmd {
  # PROMPT="%F{$fade}${ruby_version}${node_version}%F{$(itg_exit_color)}╳ %F{$normal} $(itg_dir) $(itg_git)%F{$normal}$(itg_pair)%f"
  PROMPT="
%F{$fade}%F{$(itg_exit_color)}$(itg_exit_symbol)%F{$normal} %~ $(itg_git)%F{$normal}
%F{$yellow}𝝀  %f"
  RPROMPT="%F{$fade} %t%f"
}
